...
for movies in movies_search:
    ...
    selectfirst.click()
    movietitle = driver.find_elements_by_xpath('//div[@class = "heroic-overview"]/div/div[2]/div')
    for movies in movietitle:
        #movie name
        try:
            title = movies.find_element_by_xpath('//div[@class = "title_block"]/div/div[2]/div[2]/h1').text
            print(title)
        except:
            #pass is do nothing and continue will go to the next loop iteration
            continue
        try:
            #imdb rating
            budget = movies.find_element_by_xpath('//*[@id="titleDetail"]/div[7]/')[1].text
            print(budget)
        except:
            continue
        try:
            #critic rating
            gross_usa = driver.find_element_by_xpath('//div[@class="titleReviewBarItem"]').text
            print(gross_usa)
        except:
            continue
    #add title to title dictionary
        news_dict['title'] = title
    #add imdb user rating to the imdb_userrating
        news_dict['budget'] = budget
    #add the imdb critic_ratign to the imdbcritic rating
        news_dict['USAgross'] = gross_usa
    #Add these info to the csv
        writer.writerow(news_dict.values())
driver.close()